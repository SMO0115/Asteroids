...
std::unique_ptr<IGameState> RunState::handleInput(InputManager &input_manager) {

    if ( input_manager.wasKeyPressed(Key::ESCAPE) ) return std::make_unique<PauseState>(m_game);

    return nullptr;
}


std::unique_ptr<IGameState> RunState::update(float deltatime, Engine& engine) {

    auto& game_state = m_game.getGameSession()->getComponent<GameStateComponent>();
    std::vector<std::unique_ptr<GameObject>>& game_objects = m_game.getGameObjects();
    std::vector<std::unique_ptr<GameObject>>& ui_objects    = m_game.getUIObjects();
...
void RunState::render(RenderSystem &renderer) {
