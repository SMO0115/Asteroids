...
bool AssetManager::loadTexture(RenderSystem &renderer, const std::string& id, const std::string& path) {
...
bool AssetManager::loadAnimation(const std::string& id, const std::string& path) {
    ...
    yyjson_arr_foreach(frames, idx, max, frame) {
        ...
        if (duration_val != nullptr && yyjson_is_real(duration_val)) duration = yyjson_get_real(duration_val);
